...
export default function MainScreen({ gameState: gs, onUpdate, onNavigate }: Props) {
  ...
  const handleAgeUp = () => {
    ...
    const newState = ageUp(gs); playSfx("levelup");
    onUpdate(newState);
  };

  // Rumors strip content for the panel
  const panelExtra = gs.rumors && gs.rumors.length > 0 ? (
    <p className="text-[9px] text-purple-300 mt-1 truncate" style={{ fontFamily: 'Courier New, monospace' }}>
      👂 {gs.rumors[0]}
    </p>
  ) : null;
    ...
    <GameLayout gameState={gs} panelExtra={panelExtra}>
      ...
      <div className="px-3 pt-3 pb-1 border-b border-border">
        <button
          ...
          className="w-full py-2 px-4 pixel-border-accent bg-accent/20 text-accent text-[11px] font-bold hover:bg-accent/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          style={PX}
        >
          🎂 +1 YEAR → Age {player.age + 1}
        </button>
        {gs.pendingEvent && (
          <p className="text-[9px] text-destructive text-center mt-1 ui-text">Resolve the event first!</p>
        )}
      </div>
      ...
      <div className="px-3 py-2 space-y-1 pb-20">
        {gs.eventLog.length === 0 && (
          <p className="text-[9px] text-muted-foreground ui-text py-2">Your life begins here...</p>
        )}
        {gs.eventLog.slice(0, 50).map(entry => (
          <div key={entry.id} className="fade-in">
            <div className="flex items-start gap-1 py-0.5">
              ...
              <div>
                <span className="text-[9px] text-muted-foreground ui-text mr-2">Age {entry.age}</span>
