    ...
    >
      ...
      <div className="flex gap-1 px-3 py-2 border-b border-border overflow-x-auto">
        <button
          ...
          className={`px-2 py-0.5 text-[5px] border flex-shrink-0 transition-colors ${filter === 'all' ? 'border-primary bg-primary/20 text-primary' : 'border-border text-muted-foreground hover:text-foreground'}`}
          style={PX}
        >ALL</button>
        {ROLE_GROUPS.map(g => (
          <button
            ...
            className={`px-2 py-0.5 text-[5px] border flex-shrink-0 transition-colors ${
      ...
      <div className="px-3 py-2 space-y-1 pb-20">
        {sorted.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-[6px] text-muted-foreground" style={PX}>NO ONE HERE</p>
          </div>
        ) : (
          sorted.map(person => {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      <p className="text-[6px] font-bold text-foreground" style={PX}>{person.name}</p>
                      <span className={`text-[5px] ${roleColor} ui-text uppercase flex-shrink-0 ml-2`}>{person.role}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[5px] text-muted-foreground ui-text w-8 text-right">
